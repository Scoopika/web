      ...
      {items.map((item, index) =>
        !item.disabled && item.href ? (
          <Link
            key={index}
            href={item.href}
            className={cn("flex items-center rounded-lg text-xs p-2 pl-4 pr-4 hover:bg-accent/20", {
          ...
          <span key={index} className="flex cursor-not-allowed items-center rounded-md p-2 opacity-60">
            {item.title}
          </span>
        )
      )}
